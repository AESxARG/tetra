...
const shader = () => `
  ...
  fn simulate(@builtin(global_invocation_id) gId : vec3<u32>) {
    let i = gId.x;
    if (i >= arrayLength(&pIO)) { return;}
    let time = uniforms.params1.x;
    let frame = u32(uniforms.uiState.w);
    if (i < arrayLength(&nodes)) {
      var n = nodes[i];
      let dir = normalize(n.targetPos.xyz - n.position.xyz);
      n.velocity = vec4<f32>(mix(n.velocity.xyz, dir * uniforms.params1.z, uniforms.params1.w), 0.0);
      n.position = n.position + n.velocity;
      if (frame % 8 == 0) {
        if (distance(n.position.xyz, n.targetPos.xyz) < uniforms.params1.y * 0.05) {
          n.targetPos = vec4<f32>(
            (rand(vec2<f32>(time, f32(i))) - 0.5) * uniforms.params1.y * 2.0, 
            (rand(vec2<f32>(time + 1.0, f32(i))) - 0.5) * uniforms.params1.y * 2.0, 
            (rand(vec2<f32>(time + 2.0, f32(i))) - 0.5) * uniforms.params1.y * 1.5, 
            1.0
          );
        }
      }
    ...
    if (p.life <= 0.0) {
      let nodeIdx = u32(rand(vec2<f32>(p.seed, time)) * f32(arrayLength(&nodes))) % arrayLength(&nodes);
      p.position = nodes[nodeIdx].position;
      p.velocity = vec4<f32>(
        cos(rand(vec2<f32>(p.seed, time)) * 6.28), 
        sin(rand(vec2<f32>(p.seed, time)) * 6.28), 
        (rand(vec2<f32>(p.seed + 1.0, time)) - 0.5) * 2.0, 
        0.0
      ) * ${CONFIG.explosionForce};
      p.life = 1.0;
      p.scale = 0.0;
      p.color = vec4<f32>(mix(uniforms.colorStart.rgb, uniforms.colorEnd.rgb, rand(vec2<f32>(p.seed + time, p.seed))), 1.0);
    } else {
      p.position = p.position + p.velocity;
      p.velocity = p.velocity * ${CONFIG.drag};
      p.life = p.life - ${CONFIG.decay};
      p.scale = smoothstep(0.0, 0.2, p.life);
    }
    pIO[i] = p;
  }
...
async function init() {
  ...
  function frame() {
    ...
    if (syncBuffer.buffer.length > 0 && timeSinceLast < CONFIG.motifTTL) {
      ...
      let barColor
      if (remaining > 0.5) {
        const t = (1.0 - remaining) * 2
        const r = Math.round(mix(255, 0, t))
        const g = 255
        const b = Math.round(mix(255, 200, t))
        barColor = `rgb(${r}, ${g}, ${b})`
      } else {
        barColor = `rgb(0, 255, 200)`
      }
      timerBar.style.width = `${remaining * 100}%`
      timerBar.style.backgroundColor = barColor
    ...
    const lv = mat4.multiply(mat4.ortho(-60, 60, -60, 60, 1, 200), mat4.lookAt(CONFIG.lightPos, [0, 0, 0], [0, 1, 0]))
    ...
    uData.set([dragActive, pulseTime, asp, frameCount], 52)
